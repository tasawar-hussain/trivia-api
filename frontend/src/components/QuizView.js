...
class QuizView extends Component {
          ...
          {Object.keys(categories).map((id) => {
            return (
              <div
                ...
                className='play-category'
                onClick={() =>
                  this.selectCategory({ type: categories[id], id })
                }
              >
                {categories[id]}
              </div>
  ...
  renderPlay() {
    const { previousQuestions, forceEnd, showAnswer, currentQuestion } =
      this.state;

    if (previousQuestions.length === questionsPerPlay || forceEnd) {
      return this.renderFinalScore();
    } else if (showAnswer) {
      return this.renderCorrectAnswer();
    } else if (!currentQuestion.question) {
      return <div className='quiz-question'>No question available</div>;
    } else {
      return (
        <div className='quiz-play-holder'>
